...
def get_current_node(self):
    ...
    lowest = 0
    if(self.openSet[0] == self.start):
        return self.fScore[self.start]
    else:
        for i in range(1,len(self.openSet)):
            if(self.fScore[self.openSet[i]] < self.fScore[self.openSet[i-1]]):
                lowest = self.fScore[self.openSet[i]]
        return lowest
...
def distance(self, node_1, node_2):
    ...
    node_1Coords = self.map.intersections[node_1]
    node_2Coords = self.map.intersections[node_2]
    return ((node_1Coords[0] - node_2Coords[0])**2 + (node_1Coords[1] - node_2Coords[1])**2)**0.5
...
def get_tentative_gScore(self, current, neighbor):
    ...
    return self.get_gScore(current) + self.distance(current,neighbour)
...
def record_best_path_to(self, current, neighbor):
    ...
    self.fScore[neighbor] = self.get_heuristic_cost_estimate(neighbor) + self.gScore[neighbor]
